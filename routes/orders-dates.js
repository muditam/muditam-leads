...
router.get("/api/shopify/orders-dates", async (req, res) => {
  ...
  const lastDigits = normalizedPhone.slice(-10); // more flexible match
  const headers = {
  ...
  try {
    ...
    if (customers.length === 0) {
      return res.json({ firstOrderDate: null, lastOrderDate: null });
    }

    // If multiple customers found, optionally filter further (e.g. by exact phone match)
    const customer = customers.find((c) =>
      (c.phone || "").replace(/\D/g, "").endsWith(lastDigits)
    ) || customers[0];
 

    const ordersUrl = `https://${SHOPIFY_STORE_NAME}/admin/api/${SHOPIFY_API_VERSION}/orders.json?customer_id=${customer.id}&status=any`;
    const ordersResponse = await axios.get(ordersUrl, { headers });

    const orders = ordersResponse.data.orders || [];

    if (orders.length === 0) {
    return res.json({ firstOrderDate: null, lastOrderDate: null, totalSpend: 0 });
    }
    ...
    // Calculate total spend (sum of all order totals)
    const totalSpend = orders.reduce((sum, order) => {
    const total = parseFloat(order.total_price) || 0;
    return sum + total;
    }, 0);

    return res.json({ firstOrderDate, lastOrderDate, totalSpend });
  } catch (error) {
    console.error("Shopify orders-dates error:", error.response?.data || error.message);
    return res.status(500).json({ error: "Failed to fetch order dates from Shopify." });
  }
});
