...
async function pageAndUpdateFulfillmentOnly(url, headers) {
 let fetched = 0,
   matched = 0,
   updated = 0,
   missingInDb = 0;
 ...
 while (url) {
   ...
   for (const raw of orders) {
     ...
     const fulfillmentStatus = raw?.fulfillment_status || "";
     const shopifyUpdatedAt = raw?.updated_at ? new Date(raw.updated_at) : null;


     const resUp = await ShopifyOrder.updateOne(
       { orderId },
       { $set: { fulfillment_status: fulfillmentStatus, shopifyUpdatedAt } },
       { upsert: false }
     );


     if (resUp.matchedCount) {
       matched += 1;
       if (resUp.modifiedCount) updated += 1;
     } else {
       missingInDb += 1;
     }
   }
 ...
 return { fetched, matched, updated, missingInDb };
}
...
router.post("/sync-fulfillment-last-90-days", async (req, res) => {
 try {
   ...
   const limit = Math.min(parseInt(limitInput || "250", 10) || 250, 250);
   ...
   const url =
     `${base}/orders.json?status=any&limit=${limit}` +
     `&updated_at_min=${encodeURIComponent(fromISO)}`;
...
/**
* POST /api/orders-shopify/sync-last-90-days-full
* Pulls orders from Shopify updated in last 90 days and upserts full mapped order data.
* Optional body/query:
*   - days (number, default 90)
*   - limit (number, default 250, max 250)
*/
router.post("/sync-last-90-days-full", async (req, res) => {
 try {
   const { SHOPIFY_ACCESS_TOKEN, SHOPIFY_STORE_NAME } = process.env;
   if (!SHOPIFY_ACCESS_TOKEN || !SHOPIFY_STORE_NAME) {
     return res.status(400).json({ error: "Missing Shopify env vars" });
   ...
   return res.json({
     ok: true,
     mode: "sync-last-90-days-full",
     days,
     updatedAtMin: fromISO,
     ...stats,
   });
 } catch (err) {
   console.error("sync-last-90-days-full error:", err?.response?.data || err);
   return res.status(500).json({
     error: "Failed to sync full order data for last 90 days",
     details: err?.message || err,
   });
 }
});
