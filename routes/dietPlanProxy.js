...
<body>
  ...
  (function(){
    ...
    // ---- helper to crop a canvas (used in weekly only)
    ...
    async function makePdf(){
      try{
        ...
        // --------- MONTHLY: single tall one-page PDF (no left/right spacing) ---------
        if (window.__PLAN_TYPE__ === 'Monthly') {
          ...
          // Determine page height to preserve aspect ratio at 210mm width
        ...
        // --------- WEEKLY: original multi-page logic (unchanged) ---------
        ...
        const addGroupToPdf = (indices, pageIndex) => {
          ...
          const imgs = indices.map((idx) => {
            ...
            return { canvas, ar: wmm / hmm };
          });

          let render = imgs.map(img => ({ w: contentW, h: contentW / img.ar }));
