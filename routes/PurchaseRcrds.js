...
const VENDOR_CACHE_TTL_MS = 60 * 1000; 
let vendorCache = {
  at: 0,
  byKey: new Map(), 
};
  ...
  try {
    ...
    if (vendorId || vendorName) {
      ...
      if (vendorId) {
        ...
        vendorOr.push({ vendorId: vendorId }); 
      }
...
router.patch("/:id", requireSession, async (req, res) => {
  try {
    ...
    if ("amount" in patch) patch.amount = parseAmount(patch.amount); 
...
router.post("/upload-invoice", requireSession, upload.single("file"), async (req, res) => {
  try {
...
router.post("/bulk-upload", requireSession, upload.single("file"), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: "No file uploaded" });
