...
// ─────────────────────────────────────────────────────────────
// Stages visible to ALL logged-in users
// ─────────────────────────────────────────────────────────────
...
function normalizeUploadMime(filename = "", contentType = "") {
...
// This avoids many video upload failures.
// ─────────────────────────────────────────────────────────────
router.get("/presign", requireSession, async (req, res) => {
  try {
    ...
    const safeContentType = normalizeUploadMime(filename, contentType);
        ...
        req.files.map(async (file) => {
          ...
          await s3
            .putObject({
              ...
              Body: file.buffer,
              ContentType:
                normalizeUploadMime(file.originalname, file.mimetype) ||
...
router.post("/:id/cut-upload", requireSession, async (req, res) => {
  try {
    ...
    script.cutUploadedBy = user.fullName;
