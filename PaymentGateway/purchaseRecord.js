...
// ----------------------
// MULTER → MEMORY STORAGE (Heroku-safe)
// ----------------------
...
function parseExcelLikeDate(input) {
  ...
  if (parts.length === 3) {
    ...
    if (month !== undefined) return new Date(y, month, d, 12, 0, 0);
  }
  ...
  try {
    ...
    if (!date || !category || !vendorId || !vendorName || !invoiceNo) {
      return res.status(400).json({
        message: "Missing required fields (date, category, vendor, invoiceNo)",
      });
    }
...
router.post("/vendors", async (req, res) => {
  try {
    ...
    const saved = await Vendor.create({
      name: name.trim(),
      email: email || "",
...
// =======================================================
// 8️⃣ BULK CSV UPLOAD (NO FS) – MEMORY SAFE
// =======================================================
      ...
      .on("end", async () => {
        ...
        for (let row of rows) {
          try {
            ...
            const amount = Number(
              (norm.invoice_amount || norm.amount || "0").replace(/[^0-9.]/g, "")
            );

            await PurchaseRecord.create({
              date: parsedDate,
              category: normalizeCategory(norm.category),
